...
Item {
    ...
    function getBitwardenSettings() {
        return {
            url: pluginApi?.pluginSettings?.bitwardenUrl || "",
            token: pluginApi?.pluginSettings?.apiToken || ""
        }
    }

    function saveBookmark() {
        var settings = getBitwardenSettings()
        if (!settings.url || !settings.token) {
            ToastService.showError("Bitwarden not configured")
            return
        }
        if (!urlInput.text) {
        ...
        var endpoint = isEditMode
            ? apiUrl + "/api/items/" + editBookmark.id + "/"
            : apiUrl + "/api/items/"
